...
{
    public static Dictionary<string, T> ReadToDictionary<T>(StreamReader streamReader, bool samKeyOverwriteValue = false) => new Dictionary<string, T>().ChangeFrom(ReadToList<T>(streamReader), samKeyOverwriteValue ? NotifyCollectionChangedAction.Replace : NotifyCollectionChangedAction.Add);
